...
def test_status(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.status_xml_to_dict', return_value={"status": "playing"})
...
def test_get_volume(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.volume_xml_to_dict', return_value={"level": 10})
...
def test_set_volume(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.volume_xml_to_dict', return_value={"level": 20})
    assert player.set_volume(level=20) == {"level": 20}
    mock_requests.assert_called_once_with("http://192.168.1.1:11000/Volume?tell_slaves=1&mute=0&level=20")

def test_volume_up_down(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.volume_xml_to_dict', return_value={"level": 22})
    assert player.volume_up() == {"level": 22}
    mock_requests.assert_called_with("http://192.168.1.1:11000/Volume?tell_slaves=1&mute=0&db=2")

    mocker.resetall()
    mocker.patch('bluospy.player.utils.volume_xml_to_dict', return_value={"level": 18})
...
def test_play(player, mock_requests, mocker):
    ...
    mock_requests.assert_called_once_with(f"http://192.168.1.1:11000/Play?url={stream_arg}")

def test_pause_toggle(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.status_xml_to_dict', return_value={"status": "paused"})
...
def test_stop(player, mock_requests, mocker):
    mocker.patch('bluospy.player.utils.status_xml_to_dict', return_value={"status": "stopped"})
...
def test_shuffle_repeat(player, mock_requests, mocker):
    # Shuffle
    mocker.patch('bluospy.player.utils.playlist_xml_to_dict', return_value={"shuffle": True})
    assert player.shuffle(True) == {"shuffle": True}
    mock_requests.assert_called_once_with("http://192.168.1.1:11000/Shuffle?state=1")

    # Repeat
    mocker.resetall()
    mocker.patch('bluospy.player.utils.playlist_xml_to_dict', return_value={"repeat": True})
    assert player.repeat(True) == {"repeat": True}
    mock_requests.assert_called_once_with("http://192.168.1.1:11000/Repeat?state=1")
