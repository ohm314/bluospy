...
def test_status(player, mock_requests, mocker):
    mocker.patch('bluospy.player.status_xml_to_dict', return_value={"status": "playing"})
...
def test_stop(player, mock_requests, mocker):
    mocker.patch('bluospy.player.status_xml_to_dict', return_value={"status": "stopped"})
...
def test_shuffle_repeat(player, mock_requests, mocker):
    ...
    mock_requests.assert_has_calls([call('http://192.168.1.1:11000/Shuffle?state=1'),
    ...
    mock_requests.assert_has_calls([call('http://192.168.1.1:11000/Repeat?state=1'),
        call('http://192.168.1.1:11000/Status')])
